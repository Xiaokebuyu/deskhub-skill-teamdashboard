  ...
  return (
    <div style={{ padding: "0 0 40px" }}>
      <PageHeader
        ...
        title="工作台"
        titleEm="orders"
        desc="工单流转 — 从「下期规划」到「进行中」到「定稿」。每个方案多人评测、按维度打分、自动按平均分排名。"
      />
      ...
      {/* 🔥 进行中 */}
      {active.length > 0 && (
        <WoSection num="01" title="进行中" titleEm="active" count={active.length}>
      ...
      {/* 📋 下期规划 */}
      {next.length > 0 && (
        <WoSection num="02" title="下期规划" titleEm="up next" count={next.length}>
      ...
      {/* ✅ 已完成 */}
      {done.length > 0 && (
        <WoSection num="03" title="已完成" titleEm="archived" count={done.length} collapsible defaultCollapsed>
